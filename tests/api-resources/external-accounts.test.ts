...
describe('resource externalAccounts', () => {
  test('create: only required params', async () => {
    const responsePromise = client.externalAccounts.create({ counterparty_id: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e' });
  ...
  test('update: request options and params are passed correctly', async () => {
    ...
    await expect(client.externalAccounts.update('id', {
    account_type: 'base_wallet',
    counterparty_id: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
    metadata: { foo: 'string' },
    name: 'name',
  ...
  test('completeVerification: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.externalAccounts.completeVerification('id', { amounts: [2, 4] }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(ModernTreasury.NotFoundError);
  });

  test('verify: only required params', async () => {
    const responsePromise = client.externalAccounts.verify('id', { originating_account_id: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', payment_type: 'ach' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
