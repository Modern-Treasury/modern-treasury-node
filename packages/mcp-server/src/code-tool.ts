...
const remoteStainlessHandler = async (
  ...
  MODERN_TREASURY_API_KEY: requireValue(readEnv('MODERN_TREASURY_API_KEY') ?? client.apiKey, 'set MODERN_TREASURY_API_KEY environment variable or provide apiKey client option'),
  MODERN_TREASURY_ORGANIZATION_ID: requireValue(readEnv('MODERN_TREASURY_ORGANIZATION_ID') ?? client.organizationID, 'set MODERN_TREASURY_ORGANIZATION_ID environment variable or provide organizationID client option'),
  MODERN_TREASURY_WEBHOOK_KEY: readEnv('MODERN_TREASURY_WEBHOOK_KEY') ?? client.webhookKey ?? undefined,
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = 'modern-treasury';
  ...
  try {
    const resp = await new Promise<Response>((resolve, reject) => {
      ...
      const opts = {
          ...
          ...(client.organizationID != null ? { organizationID: client.organizationID } : undefined),
