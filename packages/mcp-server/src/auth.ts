...
export const parseClientAuthHeaders = (req: IncomingMessage, required?: boolean): Partial<ClientOptions> => { if (req.headers.authorization) {
  ...
  switch (scheme) {
    case 'Basic':
     ...
     return { organizationID: rawValue.slice(0, rawValue.search(':')), apiKey: rawValue.slice(rawValue.search(':') + 1) }
    default:
...
const organizationID = Array.isArray(req.headers['x-modern-treasury-organization-id']) ? req.headers['x-modern-treasury-organization-id'][0] : req.headers['x-modern-treasury-organization-id']
const apiKey = Array.isArray(req.headers['x-modern-treasury-api-key']) ? req.headers['x-modern-treasury-api-key'][0] : req.headers['x-modern-treasury-api-key']
