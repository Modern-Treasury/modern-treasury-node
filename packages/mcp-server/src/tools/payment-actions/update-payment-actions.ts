  ...
  inputSchema: {
    ...
    properties: {
      ...
      status: {
        ...
        enum: ['pending', 'processable', 'processing', 'sent', 'acknowledged', 'failed', 'cancelled'],
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: ModernTreasury, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.paymentActions.update(id, body)));
  } catch (error) {
