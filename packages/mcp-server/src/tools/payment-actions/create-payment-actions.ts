...
export const handler = async (client: ModernTreasury, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.paymentActions.create(body)));
  } catch (error) {
    if (isJqError(error)) {
